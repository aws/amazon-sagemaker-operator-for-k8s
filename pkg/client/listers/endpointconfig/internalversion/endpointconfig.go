...
type EndpointConfigLister interface {
	...
	List(selector labels.Selector) (ret []*endpointconfig.EndpointConfig, err error)
...
// List lists all EndpointConfigs in the indexer.
func (s *endpointConfigLister) List(selector labels.Selector) (ret []*endpointconfig.EndpointConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*endpointconfig.EndpointConfig))
	})
	return ret, err
}
...
type EndpointConfigNamespaceLister interface {
	...
	Get(name string) (*endpointconfig.EndpointConfig, error)
...
// List lists all EndpointConfigs in the indexer for a given namespace.
func (s endpointConfigNamespaceLister) List(selector labels.Selector) (ret []*endpointconfig.EndpointConfig, err error) {
...
// Get retrieves the EndpointConfig from the indexer for a given namespace and name.
func (s endpointConfigNamespaceLister) Get(name string) (*endpointconfig.EndpointConfig, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(endpointconfig.Resource("endpointconfig"), name)
	}
	return obj.(*endpointconfig.EndpointConfig), nil
}
