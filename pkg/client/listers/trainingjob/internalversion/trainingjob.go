...
type TrainingJobLister interface {
	...
	List(selector labels.Selector) (ret []*trainingjob.TrainingJob, err error)
...
// List lists all TrainingJobs in the indexer.
func (s *trainingJobLister) List(selector labels.Selector) (ret []*trainingjob.TrainingJob, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*trainingjob.TrainingJob))
	})
	return ret, err
}
...
type TrainingJobNamespaceLister interface {
	...
	Get(name string) (*trainingjob.TrainingJob, error)
...
// List lists all TrainingJobs in the indexer for a given namespace.
func (s trainingJobNamespaceLister) List(selector labels.Selector) (ret []*trainingjob.TrainingJob, err error) {
...
// Get retrieves the TrainingJob from the indexer for a given namespace and name.
func (s trainingJobNamespaceLister) Get(name string) (*trainingjob.TrainingJob, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(trainingjob.Resource("trainingjob"), name)
	}
	return obj.(*trainingjob.TrainingJob), nil
}
